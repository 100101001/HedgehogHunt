...
def myAddressList():
    ...
    address_list = Address.query.filter_by(status=1, member_id=member_info.id) \
...
def myAddressSet():
    resp = {'code': 200, 'msg': '操作成功~', 'data': {}}
    req = request.values
    address_id = int(req['id']) if 'id' in req and req['id'] else 0
    ...
    address_info = Address.query.filter_by(id=address_id, member_id=member_info.id).first()
    if address_info:
        model_address = address_info
    else:
        default_address_count = Address.query.filter_by(is_default=1, member_id=member_info.id, status=1).count()
...
def myAddressOps():
    resp = {'code': 200, 'msg': '操作成功~', 'data': {}}
    req = request.values
    address_id = int(req['id']) if 'id' in req else 0
    act = req['act'] if 'act' in req else ''
    member_info = g.member_info

    if address_id < 1 or not member_info:
        resp['code'] = -1
        resp['msg'] = "系统繁忙，请稍后再试~~"
        return jsonify(resp)
    ...
    if act == "del":
        ...
        Address.query.filter_by(member_id=member_info.id) \
            .update({'is_default': 0})
...
def myAddressInfo():
    ...
    address_info = Address.query.filter_by(id=address_id).first()
    if not address_info:
        resp['code'] = -1
        resp['msg'] = "系统繁忙，请稍后再试~~"
        return jsonify(resp)
