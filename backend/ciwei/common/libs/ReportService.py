...
def blockGoods(goods_id=0, user_id=0):
    ...
    db.session.add(reported_goods.Good)
    # redis 同步可能需要，提前记录，避免后面再次查数据库
    goods_business_type = reported_goods.Good.business_type
    db.session.commit()
    # ES同步
    SyncService.syncUpdatedGoodsToES(goods_id=goods_id,
                                     updated={'report_status': 3, 'user_id': user_id})
    # redis 候选去除(底层判断，上层透明)
    SyncTasks.syncDelGoodsToRedis.delay(goods_ids=[goods_id], business_type=goods_business_type)
...
def blockReporter(goods_id=0, user_id=0):
    ...
    MemberService.blockMember(member_id=blocked_member_id, user_id=user_id, goods_id=goods_id, block_status=-1, block_reason="恶意举报")
...
def blockReleaser(goods_id=0, user_id=0):
    ...
    db.session.commit()
    SyncService.syncUpdatedGoodsToES(goods_id=goods_id,
                                     updated={'report_status': 5,  'user_id': user_id})
    # redis 候选去除(底层判断，上层透明)
    SyncTasks.syncDelGoodsToRedis.delay(goods_ids=[goods_id], business_type=goods_business_type)
    # 因为会员被拉黑了，所以相当于所有的物品都因为被举报而封锁
    MemberService.blockMember(member_id=blocked_member_id, user_id=user_id, goods_id=goods_id, block_status=0, block_reason="恶意发帖")
