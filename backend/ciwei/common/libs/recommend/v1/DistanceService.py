...
from common.models.ciwei import Goods
...
class DistanceService:
    ...
    def filterNearbyGoods(self, goods_list: List[Goods] = None, location: str = "") -> List[Goods]:
        """
        从匹配的列表中筛选出距离20m内的

        :param goods_list: [{id, lng, lat}, {id,lng,lat}]
        :param location: "上海市黄浦区南京西路399号###上海明天广场JW万豪酒店###31.23038###121.4697"
        :return: [] 符合距离条件的物品
        """
        if location == app.config['CONSTANTS']['default_lost_loc']:
            # 遗失地址不知道，那么所有匹配上的拾物都返回
            return goods_list
        found_gps = location.split("###")[-2:]
        ret_list = []
        for item in goods_list:
            if item.os_location == app.config['CONSTANTS']['default_lost_loc']:
                # 失主不知道丢哪里了
                ret_list.append(item)
                continue
            gps = item.os_location.split("###")[-2:]
            distance = self.calSimplifyDistance(lng1=eval(found_gps[1]), lat1=eval(found_gps[0]),
                                                lng2=eval(gps[1]), lat2=eval(gps[0]))
            app.logger.warn('物品' + str(item.id) + '距离配对的物品距离为: ' + str(distance))
            if distance < 20:
                # 直线距离20m内的才算数
                ret_list.append(item)
        return ret_list
